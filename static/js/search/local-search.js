    ...
    $.ajax({
      ...
      success: function (xmlResponse) {
        ...
        $input.addEventListener('input', function () {
          ...
          if (this.value.trim().length <= 0) { $('.local-search-stats__hr').hide() return } var count="0" perform local searching datas.foreach(function (data) ismatch="true" datatitle="data.title.trim().toLowerCase()" datacontent="data.content.trim().replace(/<[^">]+>/g, '').toLowerCase()
